...
function game_init(game_enemies)
  ...
  for _, start_enemy in ipairs(start_enemies) do
    local width, height = start_enemy.width, start_enemy.height
    ...
    local sprites = {}
    for _, sprite in ipairs(start_enemy.sprites) do 
      table.insert(sprites, sprite)
    end

    local max_hp, threshold, speed, sprite_defeat, sd = start_enemy.max_hp, start_enemy.threshold, start_enemy.speed, start_enemy.sprite_defeat, start_enemy.sd
    local enemy = { 
      max_hp = max_hp, 
      hp = max_hp, 
      count = 0, 
...
function game_update(dt)
  ...
  player.x = player.x + player.dx * player.speeds[player.state] * dt
  player.y = player.y + player.dy * player.speeds[player.state] * dt
  ...
  if player.state == "continue" then 
    ...
    local key = get_tilemap_key(column + 1, row)
    if tilemap[key] and tilemap[key].flag == flag_continue then
  ...
  for _, enemy in pairs(enemies) do 
    if enemy.hp > 0 then
      enemy.si = (enemy.si + enemy.sd * dt) % #enemy.sprites

      enemy.count = enemy.count + 1 * dt
